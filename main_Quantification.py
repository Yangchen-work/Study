...
def get_cities_num(net1, net2,n1,n2):
    ...
    name1=str(n1)
    name2=str(n2)
    net1_depart = net1["dep_city"].tolist()
    net1_dest = net1["arr_city"].tolist()
    net2_depart = net2["dep_city"].tolist()
    net2_dest = net2["arr_city"].tolist()
    total_cities_nodes = set(net1_depart + net1_dest + net2_depart + net2_dest)
    file = open("D:/workspace/python/test/01_实证网络_train_car_plane/net_data/city_list/"+name1+"_"+name2+"_nodes_list.txt", "w")
...
def get_single_cities_num(net, name):
    name=str(name)
    """创建每个网络总城市结点数"""
    net1_depart = net["dep_city"].tolist()
    net1_dest = net["arr_city"].tolist()
    total_cities_nodes = set(net1_depart + net1_dest )
    file = open("D:/workspace/python/test/01_实证网络_train_car_plane/net_data/city_list/"+name+"_node_list.txt", "w")
    for i in total_cities_nodes:
        file.write(i)
        file.write("\n")
    file.close()
...
def read_total_cities(strs):
    """读取每个多层网络中的结点数据"""
    file = open("D:/workspace/python/test/01_实证网络_train_car_plane/net_data/city_list/"+strs+"_nodes_list.txt", "r")
...
def read_csv(a_addr,b_addr,net_type1,net_type2):
    ...
    get_single_cities_num(b,net_type2)
    get_cities_num(a, b, net_type1, net_type2)  # 将不同网络的城市结点写入文件
    ...
    nodes_list_all, edges_list_weight = net_nodes_to_number(total_cities, weighted_net1_lines)
    net1_weighted_array = create_network_graph(nodes_list_all, edges_list_weight, net1_array)
...
def D_value(W1,W2,W3,a_addr,b_addr,net_type1,net_type2):
    total_city,g_a_G=read_csv(a_addr,b_addr,net_type1,net_type2)
    total_city,g_b_G=read_csv(b_addr,a_addr,net_type1,net_type2)#读取出图
    n_a=len(total_city)
    n_b=len(total_city)
    a_node=range(0,n_a)
    b_node=range(0,n_b)
